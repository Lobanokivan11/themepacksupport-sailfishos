...
if [ ! -f $main/backup/droiddpi ]; then
   ...
   grep "^ro.sf.lcd_density" /opt/alien/system/build.prop | tr ":" " " | egrep -o '.{1,3}$' | head -n1 > $main/backup/droiddpi
fi

# Apply Android DPI
sed -i "s/.*ro.sf.lcd_density.*/ro.sf.lcd_density=$aliensize/" /opt/alien/system/build.prop
echo "" > $main/droiddpi-current
grep "^ro.sf.lcd_density" /opt/alien/system/build.prop | tr ":" " " | egrep -o '.{1,3}$' | head -n1 > $main/droiddpi-current
